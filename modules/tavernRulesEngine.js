...
(function (root, factory) {
    ...
    function migrateLegacyAgentBubbleTheme(store, legacyEnabled) {
        const normalized = normalizeRuleStore(store);
        const divRenderRule = normalized.rules.find(function (rule) {
            return rule.builtinKey === 'agent-div-render' || rule.id === 'builtin_agent_div_render';
        });
        if (legacyEnabled !== true || !divRenderRule || divRenderRule.enabled === true) {
            return { store: normalized, changed: false, enabledOverrideCreated: false };
        }
        divRenderRule.enabled = true;
        return { store: normalized, changed: true, enabledOverrideCreated: true };
    }
