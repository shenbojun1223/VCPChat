...
function createCatalog(ops) {
    return {
        ...
        runDesktopSync: query(() => ops.invoke('desktop-sync-now')),
        getDesktopSyncStatus: query(() => ops.invoke('desktop-sync-status')),
        onDesktopSyncStatus: subscription(ops.subscribe('desktop-sync-status', (_event, status) => status)),
        onDesktopSyncDataUpdated: subscription(ops.subscribe('desktop-sync-data-updated', (_event, status) => status)),
