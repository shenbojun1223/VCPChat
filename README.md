...
*   **高级能力渲染**: VCPChat 专为渲染 VCP 协议的复杂输出而设计。统一的 **VCPMessageRenderer V4** 不只负责“显示一段回复”，而是把每条消息视为可持续生成、局部更新、深层嵌套并拥有独立生命周期的微型应用。无论是 AI 主动记录和反思的“日记”、复杂文档、多媒体内容，还是脚本动画与交互界面，都能在流式输出过程中稳定呈现。
...
*   **🖥️ VCPDesktop — AI 原生桌面**:
    ...
    *   **高性能流式渲染**: 依托 VCPMessageRenderer V4、涟漪渐进队列与墓碑冻结 V2，优先渲染当前可见内容，并冻结离屏动画帧、计时器与活动节点，使复杂动态桌面和长时间运行场景仍能保持极低的无效 CPU / GPU 开销。
...
*   **聊天界面**:
    *   提供用户友好的聊天界面进行 AI 交互。
    *   统一支持 Markdown、KaTeX / LaTeX、HTML、Mermaid、VCPTool、Manim、Matplotlib、Anime.js、Three.js、交互式按钮与弹窗、Div 岛、远程资源、draw.io、CSV、PDF 等三十余类内容管线，并允许它们在同一条流式消息中交叉组合、递归嵌套和按依赖渐进富化，包括持续准确的代码块高亮。
    *   **强大的多媒体与文件处理**:
    ...
    *   **语义级 Div 岛与极端内容防御**: V4 不再依赖脆弱的标签表面匹配，而是识别复杂动态内容的真实作用域。即使流式片段中出现未闭合或非规范嵌套的 `div`、`script`、`style`，混入注释伪标签、无关 Div 字段、破坏性占位符或代码示例，也能将其约束在正确的消息与隔离作用域内。Python 运行结果、远程图片、Markdown 文档、表格、Anime.js 和 Three.js 等内容可以在同一 Div 岛中按依赖顺序渐进拼装；异常中间态不会污染其它气泡或主界面，最终再统一收敛到规范终稿。
...
*   **VCPMessageRenderer V4：把流式对话变成高性能应用运行时**:
    *   **技术核心**: V4 已经超越传统 Markdown 渲染器或“打字机效果”。它以消息身份、流式代次和局部内容块为最小更新单位，在 Chat Core、渲染队列、不同话题及多个窗口 / Surface 之间维持统一状态；底层聊天记录发生变化时，只对真正变化的局部 DOM 做外科手术式更新，不刷新整页、不回灌整条消息，也不让旧异步任务覆盖新画面。
    *   **从竞态治理到可验证终稿**:
        *   **流式 ID、代次与队列序列化**: 前后台流式会话、消息身份与涟漪任务采用统一校验规则。高速分包、网络抖动、重试、话题切换、窗口迁移和迟到事件同时发生时，旧任务也无法越过新代次写入 DOM，从根源上抑制串线、重复推进、局部回退和重复结束。
        *   **预览—终稿双阶段提交**: 流式阶段以轻量管线持续推进文字与局部结构；结束后统一切换到规范终稿，并撤销仍在排队的旧任务。Markdown、公式、代码、样式和复杂富内容最终共享同一个可验证结果，不再出现“流式时正确、结束后变样”或后台任务篡改终稿的问题。
        *   **职责解耦与多 Surface 一致性**: 流式生产、Chat Core 状态、界面投影、内容富化和历史保存拥有明确边界。主聊天、独立窗口、VoiceChat、Rust Assistant 以及 V阅读、V论坛、V笔记、V文坊可以复用同一 V4 内核，同时保持各自独立的会话身份和生命周期。
    *   **性能已经从“少刷新”进化到“尽量不做无效工作”**:
        *   **块级纯增量 O(n) 管线**: 消息被拆分为可独立哈希、缓存和富化的内容块；稳定内容直接复用，活跃尾部只处理新增变化。Stable / Stream 滑动 AST 窗口会持续把已稳定区域移出高频解析路径，使超长复杂消息摆脱反复全量扫描造成的 O(n²) 放大。
        *   **涟漪渐进渲染（Ripple Rendering）**: 多级优先队列首先锁定用户当前可见范围，优先完成布局尺寸与基础文本，再按距离和内容成本向外渐进富化。切换界面或中断生成时，未产生用户价值的任务会被直接中止和回收，避免后台渲染洪峰与内存堆积。
        *   **墓碑冻结 V2（Tombstone Freeze V2）**: 富内容出生即冻结，仅在进入视野时解冻。离屏后，系统不只是隐藏 DOM，还会暂停动画帧、计时任务、事件活动和更多隐藏在复合样式与脚本调度中的潜在动画；暂停态由轮询改为事件唤醒，让上千层、每层近似一个 WebApp 的历史消息不再持续偷吃 CPU / GPU。
        *   **多级缓存与 Worker 化**: Raw HTML、块级结果、DOM 后处理和管线指纹进入受控 LRU / WeakMap 缓存；超长历史的文本到 HTML 解析与终稿计算可转移到 Worker。主线程因此主要处理当前可见画面，而不是重复计算已经完成的历史内容。
        *   **PreText / PreDOM 布局预计算**: 文本与多模态节点可在挂载前预估尺寸、排版边界和 Canvas 占位，配合消息级固有尺寸索引降低布局抖动。滚动器同时收敛非整数 Windows 缩放下的像素计算，避免细微误差演变成消息窗口全局重排。
    *   **复杂内容也有严格边界**:
        *   **语义级 Div 岛**: 深层嵌套、非规范标签、动画脚本和多渲染器混排被约束为具有明确作用域的内容岛，支持 `CSS + Markdown`、`Anime.js + Three.js`、`Python + HTML` 等组合边生成、边执行、边展示。
        *   **样式与执行隔离**: ScopedCSS 只在需要防御的块级边界工作；HTML 预览、代码示例、脚本、注释伪标签和异常占位符不会被误认成主界面结构。单条消息再极端，也不应把样式或错误 DOM 泄漏到其它消息。
        *   **完整生命周期回收**: 动画帧、定时器、观察器、预览窗口、工具结果缓存与异步任务均随消息和 Surface 生命周期有序释放，界面销毁后返回的迟到结果会被拒绝，从而保持长时间运行的稳定性。
