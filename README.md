...
### 🕸️ VCP Loom — 将整个 Web 编织为 Agent 的原生能力 (2026.08.09)

VCPChat 正式推出 **VCP Loom 系统**：一种面向 AI Agent 的网页运行时封装、应用化与自主操作引擎。Loom 不再把网页视为只能被人浏览的页面，而是允许 Agent 将**任意 Web 运行时封装为 AI 专属的独立 LoomAPP**，作为自身可长期使用、可感知、可编辑、可操作的额外功能。

这意味着，世界上已经存在的海量 Web 应用、在线服务和硬件控制页面，都可以直接成为 Agent 的能力组件。无需等待传统插件重写接口，也无需为每个网站重新开发一套原生客户端：Agent 可以把网页“编织”成应用，并持续理解、操控和改造它。

LoomAPP 的网页智能并非依赖普通浏览器脚本拼凑，而是完整调用 **VCP 自研的 VCP Agent WebCore**。它在 Agent 与复杂网页之间建立了一层 AI 原生语义运行时：Agent 无需学习 HTML、CSS、DOM 层级或网站内部实现，看到的是经过理解、降噪和定位后的**树状 Grounded Markdown 文档**，操作的则是与自然语义绑定的可靠目标。

*   **任意 Web 运行时应用化**: 只需提供目标网址，Agent 即可创建拥有独立名称、图标、窗口、视口和启动配置的 LoomAPP。网页级搜索引擎、网页级 Agent 框架、在线 IDE、数据后台、社交网站、内容平台、智能硬件控制台等，都能成为 AI 的独立附加能力。
*   **Agent 自主创建与维护**: Agent 可通过 VCP Loom 控制器列出、创建、打开、关闭、查询和编辑 LoomAPP，并可实时修改应用清单、辅助样式与注入脚本。应用运行时支持热更新，让 Agent 能够在使用过程中持续优化自己的工具。
*   **VCP Agent WebCore 原生驱动**: 每一个运行中的 LoomAPP 都接入 VCP 自研 WebCore。WebCore 自动分析网页的视觉区域、正文层级、控件关系、交互语义和页面状态，再生成树状 Grounded Markdown、交互树与页面图谱。对 Agent 而言，网页不再是一片冗长杂乱的 DOM 源码，而是一份可以快速阅读、准确理解的自然语义文档。
*   **无需理解 HTML 与 DOM**: Agent 无需解析标签、选择器、样式类名或前端框架内部结构。按钮、链接、输入框、列表、标题、正文区域等元素会被转译为具有清晰角色、文本含义和上下文关系的语义节点，并绑定稳定的动作句柄，大幅减少网页理解所需的 Token、推理步骤和误操作。
*   **强力网页降噪与广告过滤**: WebCore 会识别广告、推广、赞助内容、装饰图片、无效导航及其他低价值噪声，优先提取真正的正文、关键控件和内容区域。即使面对结构复杂、广告密集或由大型前端框架动态生成的网站，Agent 仍能快速抓住页面重点。
*   **内置多模态网页识别**: 除文字与交互控件外，WebCore 还能识别正文插图、视频当前画面及其视觉位置，为关键多媒体生成可追踪的图片 ID。Agent 可按语义选择特定图片或视频画面，获取实际渲染截图并进行视觉理解，而无需盲目截取整张网页。
*   **以自然语言为核心的操作方式**: Agent 主要依据页面呈现的自然语言、控件角色和上下文含义决定操作目标，例如“点击登录”“在搜索框输入关键词”“选择指定选项”或“滚动到结果区域”，无需手写 DOM 查询代码。WebCore 会把这些意图落实为点击、输入、选择、悬停、滚动、按键和等待等标准动作。
*   **步进式连续指令**: Loom 支持把点击、输入、等待、重新感知等动作组织成按编号执行的连续步骤，公共应用上下文可自动继承；任一步失败都会停止后续操作。Agent 因而可以像人一样自然地完成“打开搜索页 → 输入问题 → 提交 → 等待加载 → 读取结果”这类完整流程，并在关键步骤重新获取页面快照校验状态。
*   **快照驱动的可靠交互**: WebCore 为页面文档代次、快照和交互目标建立严格身份。Agent 在操作前读取语义快照，使用其中的 Grounded 句柄执行动作，再获取新快照确认页面变化，有效避免动态网页中因节点变化、导航跳转或旧引用失效造成的误点。
*   **深层网页运行时控制**: 除自然语义操作外，Loom 还向受控 Agent 提供运行时脚本、深层 DOM、无障碍树、可信原生输入、网络、存储、仿真、截图、调试器与 CDP 等多层能力。自然语言交互可以覆盖绝大多数日常任务，必要时又能深入复杂 WebApp 的内部状态。
*   **独立身份与持久会话**: 每个 LoomAPP 使用独立持久化会话和存储分区，可分别保留登录状态、Cookie、缓存、授权与网站数据。不同应用之间互不污染，使 Agent 可以像拥有多部独立设备一样长期运行自己的专属应用。
*   **可定制运行环境**: 每个应用可配置桌面、移动端、iPhone 或自定义请求特征，设置独立 User-Agent、请求头、窗口尺寸、固定或自适应视口、弹窗策略和跨域导航策略，并通过注入样式与脚本把通用网页改造成真正适合 AI 与用户共同使用的专属界面。
*   **网页级硬件驱动**: Loom 原生打通 WebHID、WebUSB、WebSerial 与 WebBluetooth。支持这些标准的厂商网页控制台可以被封装为硬件 LoomAPP；HID、USB 与串口设备在用户选择后可持久保存授权，蓝牙设备则遵循 WebBluetooth 的即时选择流程。Agent 由此可在可信边界内操作键盘、开发板、串口设备、USB 外设、蓝牙设备及其他物理硬件。
*   **运行时源码与渲染结果读取**: Agent 可查询 LoomAPP 保存的配置和注入源码，也可读取当前运行页面经过脚本与注入处理后的即时 DOM、可见文本、页面标题、URL 和成功渲染快照，从而诊断应用、理解状态并自主修复。
*   **安全边界与用户主权**: Loom 使用上下文隔离、沙箱、来源校验、严格页面句柄、文档代次与快照校验来降低误操作风险；敏感网页内容默认脱敏，硬件访问必须经过设备选择与授权。深层动作的权限等级、安全审核和用户审批仍由 VCP 引擎统一管理。
*   **可分发的 AI 应用生态**: LoomAPP 可导入、导出为独立 VCP Loom 分发包，携带应用清单、注入样式、脚本与图标。Agent 和用户可以分享经过适配的 AI 专属应用，让一次封装成为整个 VCP 生态可复用的能力。

#### Loom 能把什么交给 Agent？

*   **网页级搜索引擎**: 将专业搜索、学术检索、企业知识检索或垂直行业数据库封装为长期可用的 Agent 搜索器。
*   **网页级 Agent 框架**: 将已有的在线 Agent 平台、自动化工作台、多模型控制台封装为“Agent 调用 Agent”的专属应用。
*   **网页级硬件驱动**: 将支持 Web 标准的设备配置页和厂商控制台封装为硬件能力，让 Agent 从信息世界延伸到物理世界。
*   **网页级社交网络**: 将论坛、社区、协作平台和社交网站封装为 Agent 的独立社交入口，使其能够在用户授权下阅读、交流与协作。
*   **网页级生产力工具**: 在线文档、设计器、表格、代码编辑器、数据看板、音视频工作台和云服务控制台，都可以直接进入 Agent 的工具链。
*   **任何尚未被插件化的 Web 能力**: 只要服务能够在现代 Web 运行时中工作，Loom 就为它提供一条进入 AI 原生世界的路径。

> **Loom 的目标不是再造一个浏览器，而是把整个 Web 变成 Agent 的应用商店、工具箱与硬件总线。** VCP Agent WebCore 负责把复杂网页转译为 AI 能快速理解的树状自然语义文档，Loom 则负责把它封装成可持久使用的独立应用。从网页信息检索到多 Agent 编排，从社交协作到物理设备控制，Agent 可以自然、高效地创建、理解、维护并操作属于自己的应用——让每一个 Web 运行时都成为 AI 能力的一部分，让 Agent 真正无所不能。
